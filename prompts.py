...
Max Score: {max_score}
Vraag: {exam_question}
Brontekst (indien relevant): {source_text_snippet}
Opties:
{options_text}
...
3.  **Uitleg Correct Antwoord:**
    ...
    *   Verwijs **specifiek naar de relevante zin(nen) of passage(s)** in de brontekst (`{source_text_snippet}`) of de logica van de vraag (`{exam_question}`) die dit bewijzen.
...
2.  **Feedback Geven per Bewering (DIEPGAAND & GESTRUCTUREERD!):** Geef na de status, voor **elke bewering**, feedback in een **genummerde lijst**:
    ...
    *   `**Uitleg & Onderbouwing:**` Leg **altijd grondig uit WAAROM** het correcte antwoord (`Wel` of `Niet`) juist is. Verwijs **zeer specifiek naar de relevante zin(nen) of passage(s)** in de brontekst (`{source_text_snippet}`) die dit bewijzen of weerleggen. Citeer eventueel kort het cruciale deel.
...
2.  **Feedback Geven (UITGEBREID EN LEERZAAM!):** Geef na het statuswoord altijd gedetailleerde feedback:
    ...
    *   **Uitleg:** Leg **altijd grondig uit WAAROM** het correcte antwoord juist is. Verwijs **zeer specifiek naar de relevante zin(nen) of passage(s)** in de brontekst die dit ondersteunen. Citeer eventueel kort de context.
...
2.  **Feedback Geven (DIEPGAAND & GESTRUCTUREERD!):** Geef na de status **altijd** gedetailleerde, stapsgewijze feedback:
    ...
    *   `**Uitleg & Onderbouwing:**` Dit is het belangrijkste deel! Leg **altijd grondig uit WAAROM** het antwoord (in)correct of gedeeltelijk is. Ga dieper dan alleen constateren:
        *   Onderbouw **elk punt** uit de vergelijking met **specifieke verwijzingen naar de brontekst** (`{source_text_snippet}`). Citeer relevante zinsdelen.
...
- Snippet Brontekst (indien relevant): {source_text_snippet}
...
2.  **Feedback Geven (DIEPGAAND & GESTRUCTUREERD!):** Geef na de status altijd gedetailleerde feedback:
    ...
    *   `**Uitleg & Onderbouwing:**` Leg **altijd uit WAAROM** het correcte citaat het antwoord op de vraag (`{exam_question}`) is. Welke **specifieke woorden of informatie** in het citaat zijn cruciaal? Verwijs expliciet naar de vraagstelling.
...
2.  **Feedback Geven (DIEPGAAND & GESTRUCTUREERD!):** Geef na de status altijd gedetailleerde feedback:
    ...
    *   `**Correct Antwoord:**` [**{correct_antwoord}**]
    *   `**Uitleg & Onderbouwing:**` Leg **altijd duidelijk uit HOE** je aan het juiste antwoord komt. Verwijs **specifiek naar de relevante zin(nen) of passage(s)** in de tekst (`{source_text_snippet}`) waar de informatie te vinden is of geteld moet worden. Citeer de relevante delen kort.
...
2.  **Feedback Geven (DIEPGAAND & GESTRUCTUREERD!):** Geef na de status altijd gedetailleerde feedback:
    ...
    *   `**Uitleg & Onderbouwing:**` Leg **altijd grondig uit WAAROM** de correcte volgorde logisch is. Verwijs naar **specifieke indicatoren** zoals:
        ...
        *   Indien van toepassing, link naar de **brontekst**.
...
Taak: Beantwoord de vervolgvraag van de leerling kort en duidelijk. Verwijs indien nodig naar de context (vraag, model, feedback).
...
- Brontekst (fragment met gaten, bijv. __40-1__): {source_text_snippet} # Indien relevant
